...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        labels = list(train.tags.string_vocab)
        self._o_index = labels.index("O")

        def allowed(previous: str, current: str) -> bool:
            if previous in {MorphoDataset.PAD, "[PAD]", "[UNK]"} or current in {"[PAD]", "[UNK]"}:
                return False
            if current == "O" or current.startswith("B-"):
                return True
            if current.startswith("I-"):
                entity = current[2:]
                return previous in {f"B-{entity}", f"I-{entity}"}
            return False

        A = torch.tensor(
            [[allowed(prev, curr) for curr in labels] for prev in labels], dtype=torch.bool,
        )
    ...
    def constrained_decoding(self, logits: torch.Tensor, word_ids: torch.Tensor) -> torch.Tensor:
        ...
        neg_inf = torch.tensor(-1e9, dtype=emissions.dtype, device=emissions.device)
        emissions[:, :, MorphoDataset.PAD] = neg_inf

        transition_scores = torch.where(self._A, torch.zeros_like(self._A, dtype=emissions.dtype), neg_inf)
        start_scores = torch.where(
            self._A[self._o_index], torch.zeros(num_tags, dtype=emissions.dtype, device=emissions.device), neg_inf,
        )

        scores = emissions[:, 0] + start_scores
        history = [scores]
        backpointers = [torch.zeros(batch_size, num_tags, dtype=torch.long, device=emissions.device)]

        for timestep in range(1, max_length):
            candidates = scores.unsqueeze(2) + transition_scores.unsqueeze(0)
            best_scores, best_previous = candidates.max(dim=1)
            scores = emissions[:, timestep] + best_scores
        ...
        current_tags = history[lengths - 1, batch_indices].argmax(dim=1)

        predictions = torch.full((batch_size, max_length), MorphoDataset.PAD, dtype=torch.long, device=logits.device)
        for timestep in range(max_length - 1, -1, -1):
            active = lengths > timestep
            predictions[active, timestep] = current_tags[active]
            if timestep:
                current_tags = backpointers[timestep, batch_indices, current_tags]
        return predictions
    ...
    model.configure(
        ...
        metrics={
            ...
            "f1_constrained": npfl138.metrics.BIOEncodingF1Score(
                list(morpho.train.tags.string_vocab), ignore_index=morpho.PAD,
            ),
