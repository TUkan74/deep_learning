...
class SelfAttentionBlock(torch.nn.Module):
    def __init__(self, channels: int, heads: int) -> None:
        ...
        groups = max(1, min(channels // 4, 16))
        self._norm = torch.nn.GroupNorm(groups, channels)
        self._attention = torch.nn.MultiheadAttention(channels, heads, batch_first=True)

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        # TODO: Implement the forward pass of the self-attention block.
        batch_size, channels, height, width = inputs.shape
        sequence = self._norm(inputs).movedim(1, -1).reshape(batch_size, height * width, channels)
        attended, _ = self._attention(sequence, sequence, sequence, need_weights=False)
        attended = attended.reshape(batch_size, height, width, channels).movedim(-1, 1)
        result = inputs + attended
...
class DownscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int, attention_heads: int | None = None) -> None:
        ...
        self._attention = SelfAttentionBlock(width, attention_heads) if attention_heads is not None else None
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, channels: int, stage_blocks: int, stages: int, attention_stages: int, attention_heads: int):
        ...
        self._initial = torch.nn.Conv2d(Image64Dataset.C, channels, kernel_size=3, padding="same")
        self._downscaling_blocks = torch.nn.ModuleList(
            DownscalingBlock(
                stage_blocks,
                channels << stage,
                attention_heads if stage >= stages - attention_stages else None,
            )
        ...
        self._middle_attention = SelfAttentionBlock(channels << stages, attention_heads)
        self._upscaling_blocks = torch.nn.ModuleList(
            UpscalingBlock(
                stage_blocks,
                channels << stage,
                attention_heads if stages - 1 - stage < attention_stages else None,
            )
    ...
    def forward(self, images: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
        ...
        for block in self._middle_blocks:
            hidden = block(hidden, times)
        hidden = self._middle_attention(hidden)
...
class FlowMatching(npfl138.TrainableModule):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self._model = UNet(
            args.channels, args.stage_blocks, args.stages, args.attention_stages, args.attention_heads
        )
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        predictions = self._model(noisy_images, times)
    ...
    def generate(self, initial_noise: torch.Tensor, steps: int) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        for step in range(steps):
            ...
            images = images + self._ema_model(images, times) / steps
