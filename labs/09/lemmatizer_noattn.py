...
class Model(npfl138.TrainableModule):
    ...
    def encoder(self, words: torch.Tensor) -> torch.Tensor:
        ...
        _, states = self._source_rnn(packed)
    ...
    def decoder_training(self, encoded: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        lengths = (targets != MorphoDataset.PAD).sum(dim=1).cpu()
        embedded = self._target_embedding(inputs)
        packed = torch.nn.utils.rnn.pack_padded_sequence(
            embedded, lengths, batch_first=True, enforce_sorted=False,
        )
        packed, _ = self._target_rnn(packed, encoded.unsqueeze(0))
        hidden, _ = torch.nn.utils.rnn.pad_packed_sequence(
            packed, batch_first=True, total_length=targets.shape[1],
        )
