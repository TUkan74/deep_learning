...
class WithAttention(torch.nn.Module):
    ...
    def setup_memory(self, encoded):
        ...
        self._encoded_mask = encoded.abs().sum(dim=-1) == 0
    ...
    def forward(self, inputs, states):
        ...
        decoder_projected = self._project_decoder_layer(states).unsqueeze(1)
        logits = self._output_layer(torch.tanh(self._encoded_projected + decoder_projected)).squeeze(-1)
        logits = logits.masked_fill(self._encoded_mask, -1e9)
...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        self._source_embedding = torch.nn.Embedding(
            len(self._source_vocab), args.cle_dim, padding_idx=MorphoDataset.PAD,
        )
        ...
        self._target_rnn_cell = WithAttention(
            torch.nn.GRUCell(args.cle_dim + args.rnn_dim, args.rnn_dim), attention_dim=args.rnn_dim,
        )
        ...
        if not args.tie_embeddings:
            ...
            self._target_embedding = torch.nn.Embedding(
                len(self._target_vocab), args.cle_dim, padding_idx=MorphoDataset.PAD,
            )
        else:
            ...
            self._target_embedding = lambda inputs: torch.nn.functional.embedding(
                inputs, self._target_output_layer.weight * args.rnn_dim ** 0.5,
    ...
    def encoder(self, words: torch.Tensor) -> torch.Tensor:
        ...
        packed, _ = self._source_rnn(packed)
        encoded, _ = torch.nn.utils.rnn.pad_packed_sequence(
            packed, batch_first=True, total_length=words.shape[1],
        )
        return encoded[:, :, :self._source_rnn.hidden_size] + encoded[:, :, self._source_rnn.hidden_size:]
    ...
    def decoder_training(self, encoded: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        inputs = torch.cat([
            torch.full((targets.shape[0], 1), MorphoDataset.BOW, dtype=torch.long, device=targets.device),
            targets[:, :-1],
        ], dim=1)
        ...
        for timestep in range(embedded.shape[1]):
            states = self._target_rnn_cell(embedded[:, timestep], states)
            outputs.append(states)
        hidden = torch.stack(outputs, dim=1)
        return self._target_output_layer(hidden).permute(0, 2, 1)
    ...
    def decoder_prediction(self, encoded: torch.Tensor, max_length: int) -> torch.Tensor:
        ...
        while index < max_length and torch.any(result_lengths == max_length):
            ...
            hidden = self._target_rnn_cell(self._target_embedding(inputs), states)
            states = hidden
            predictions = self._target_output_layer(hidden).argmax(dim=-1)
...
class TrainableDataset(npfl138.TransformedDataset):
    ...
    def collate(self, batch):
        ...
        words = torch.nn.utils.rnn.pad_sequence([
            torch.tensor(self.dataset.words.char_vocab.indices(word), dtype=torch.long)
            for sentence in words for word in sentence
        ], batch_first=True, padding_value=MorphoDataset.PAD)
        ...
        lemmas = torch.nn.utils.rnn.pad_sequence([
            torch.tensor(
                self.dataset.lemmas.char_vocab.indices(lemma) + [MorphoDataset.EOW], dtype=torch.long,
            )
        ...
        inputs = (words, lemmas) if self._training else words
